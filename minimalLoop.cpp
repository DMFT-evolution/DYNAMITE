               ...
               const double t)
{
    ...
    // 2) preallocate and resize to accommodate new data
    size_t oldSize = QKv.size();
    QKv.  resize(oldSize + length);
    QRv.  resize(oldSize + length);
    dQKv. resize(oldSize + length);
    dQRv. resize(oldSize + length);

    // 3) write directly to the new locations (thread-safe because segments do not overlap)
    #pragma omp parallel for
    for (long int i = 0; i < (long int)length; ++i) {
        size_t pos = oldSize + i;
        QKv[pos]   = qK[i];
        QRv[pos]   = qR[i];
        dQKv[pos]  = tdiff * dqK[i];
        dQRv[pos]  = tdiff * dqR[i];
    }
...
vector<double> bsearchPosSorted(const vector<double>& list, const vector<double>& elem) {
    ...
    for (size_t l = 0; l < elem.size(); ++l) {
        El = elem[l];
        n0 = 0; // Start index (Mathematica uses 1-based indexing, C++ uses 0-based)
        n1 = list.size() - 1; // End index
        m = 0; // Midpoint index
        Lm = 0.0;
        ...
        n1 = list.size() - 1; // Reset n1
        if (Lm <= El) {
            if (m == list.size() - 1) {
                result[l] = m;
            } else {
                result[l] = m + (El - Lm) / (list[m + 1] - Lm);
            }
        } else {
            if (m > 0) {
                result[l] = m - (El - Lm) / (list[m - 1] - Lm);
...
double SSPRK104() 
{
    ...
    for (int n = 0; n < stages; ++n) {
        ...
        if (QKv.size() == len || n != 0) {
            interpolate(
                (n == 0 ? vector<double>{0.}  : (n == 5 ? posB1xvec[0] : (n == 6 ? posB1xvec[1] : (n == 7 ? posB1xvec[2] : posB1xOld)))),
                (n == 0 ? vector<double>{0.}  : (n == 5 ? posB2xvec[0] : (n == 6 ? posB2xvec[1] : (n == 7 ? posB2xvec[2] : posB2xOld)))),
                (n == 5 || n == 6 || n == 7)
            );
        }

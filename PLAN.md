...
## Implementation notes (diagram accuracy)

### Method diagram (`diagram-method.svg`)
Based on careful analysis of `src/simulation/simulation_runner.cpp`, `src/EOMs/runge_kutta.cpp`, and `src/EOMs/time_steps.cpp`:

**Main loop structure:**
- `while (t < tmax && loop < maxLoop && delta_t >= delta_t_min)`
- Each iteration: `update()` or `updateGPU()` → dispatches to RK54, SSPRK104, or SERK2

**Inside each RK method** (multi-stage):
- 7 stages for RK54, 10 for SSPRK104
- Each stage: `interpolate()` → `QKstep()` → `QRstep()` → `appendAll()`/`replaceAll()`
- QKstep/QRstep compute dC/dt and dR/dt via convolutions (ConvA, ConvR with history)

**Periodic operations (every 100k iterations):**
- `sparsifyNscale()` prunes grid points
- `interpolate()` rebuilds interpolation tables

**Adaptive timestep logic:**
- Grow by 1.01 if error (delta) is small
- Shrink by 0.9 if error is too large
- Method switching: SSPRK104 → RK54 if `delta > delta_max`
- GPU-only: rollback state if `delta > 2*delta_max`

The diagram accurately depicts these feedback loops, conditionals, and the nested structure.

### Architecture diagram (`diagram-architecture.svg`)
- Top: Simulation control (CLI, scheduling, checkpoints, I/O coordination)
- Middle: Computational pipeline: EOMs → Integrator → Interpolation → Convolution
- Bottom: Support modules: Sparsify (grid pruning) and Core (memory, GPU/CPU kernels)
- Arrows show data flow through stages and periodic sparsification calls
